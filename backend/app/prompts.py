...
SPLIT_DETECTION_USER_PROMPT = (
    "请分析这段视频（总时长 {duration:.2f} 秒），输出切割时间点列表。\n"
    "\n"
    "注意：\n"
...
EFFECT_ANALYSIS_SYSTEM_PROMPT_TEMPLATE = (
    "You are a visual effects analyst specialized in identifying "
    "UI/motion design effects in short-form videos.\n"
    "\n"
    "Your task is to analyze the provided video and identify which "
    "visual effects from the given effects library are present.\n"
    "\n"
    "## Effects Library\n"
    "\n"
    "{effects_library}\n"
    ...
    "you observe. Focus strictly on HOW things look and move, not WHAT "
    "the content is about.\n"
    "\n"
    "Observe along these dimensions:\n"
    "- **Text elements**: How do they appear or disappear? "
    "Character-by-character, word-by-word, fade, slide? "
    "Any cursor or highlight?\n"
    "- **Screen motion**: Does the camera move? "
    "Do elements translate, scale, or rotate?\n"
    "- **Visual anomalies**: Any blur, noise, distortion, "
    "tearing, color shift, or pixelation?\n"
    "- **Timing patterns**: Is the animation a one-shot entry, "
    "a loop, or an exit? Fast flash or slow transition?\n"
    "- **Overlay elements**: Any particles, light leaks, grain, "
    "or texture layers on top of the image?\n"
    "\n"
    "Be specific and visual. Do NOT reference effect names in this step.\n"
    "\n"
    "### Step 2: Match to Effects Library\n"
    "Using ONLY what you described in Step 1 as evidence, match "
    "observations to effects in the library.\n"
    "\n"
    "Rules:\n"
    "- Every matched effect must have a corresponding observation "
    "from Step 1\n"
    "- If you cannot point to a specific visual phenomenon as "
    "evidence, do NOT include the effect\n"
    "- Prefer under-reporting over over-reporting — only include "
    "effects you are confident about\n"
    "- An effect that is merely \"possible\" or \"suggested\" "
    "should be excluded\n"
)
