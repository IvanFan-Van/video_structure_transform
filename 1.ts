...
/** Parse a non-success API response body into a NodeError-ready triplet. */
...
/** POST /api/task/:id/cancel — best-effort, always swallows errors. */
...
interface TaskStreamOptions<TResult> {
    /** Key into streamControllers */
    controllerKey: string;
    /** Timer start — Date.now() */
    t0: number;
    /** Zustand set */
    set: (updater: any) => void;
    /** nodeId used for error attribution and videoErrors filtering */
    nodeId: string;
    /** Human-readable label for task failure, e.g. "Compress failed" */
    failureLabel: string;
    /**
     * Called for every parsed frame before terminal-state handling.
     * Use for intermediate progress updates such as streaming audio frames.
     * The callback receives the raw parsed frame and the current Zustand state.
     */
    onFrame?: (frame: any, state: any) => void;
    /** Partial state to merge when the task completes successfully.
     *  Receives current state so callers can derive values from it (e.g. fallback fields). */
    onCompleted: (result: TResult, elapsed: number, state: any) => object;
    /** Partial state to merge when the task fails */
    onFailed: (elapsed: number) => object;
    /** Partial state to merge when the task is cancelled server-side */
    onCancelled: () => object;
}

/**
 * Connects to /api/task/:taskId/stream and handles state transitions.
 *
 * Accepts both response formats transparently:
 *   • SSE   — lines prefixed with "data: "  (text/event-stream)
 *   • NDJSON — bare JSON objects, one per line
 *
 * Lifecycle:
 *   onFrame     → called for every frame before terminal check (optional)
 *   completed   → onCompleted + clears node errors
 *   failed      → onFailed   + appends task error
 *   cancelled   → onCancelled
 *   AbortError  → silently returns (stop action already reset state)
 *   network err → onFailed   + appends network error
 */
    ...
    const setNetworkError = (elapsed: number) => {
        set((s: any) => ({
            ...
            videoErrors: [
                ...
                makeError(nodeId, "Network error", "NETWORK_ERROR", NETWORK_ERROR_DETAILS),
            ],
        }));
    };
    ...
    try {
        ...
        outer: while (true) {
            ...
            for (const raw of lines) {
                const line = raw.trim();
                // Skip blank lines and SSE metadata fields (event:, id:, retry:, comments)
                if (!line || line.startsWith(":") || /^(event|id|retry):/.test(line)) continue;

                // Normalise: strip "data: " prefix if present (SSE), otherwise use as-is (NDJSON)
                const jsonStr = line.startsWith("data:") ? line.slice(5).trim() : line;
                if (!jsonStr) continue;

                try {
                    const frame: TaskInfo = JSON.parse(jsonStr);
                    const elapsed = (Date.now() - t0) / 1000;

                    // Let the caller process intermediate frame data (e.g. audio chunks)
                    // before we check for terminal states.
                    if (opts.onFrame) {
                        set((s: any) => { opts.onFrame!(frame, s); return {}; });
                    }

                    if (frame.status === "completed") {
                        set((s: any) => ({
                            ...opts.onCompleted(frame.result as TResult, elapsed, s),
                        ...
                        set((s: any) => ({
                            ...
                            videoErrors: [
                                ...
                                makeError(nodeId, failureLabel, "TASK_FAILED", frame.error || ""),
                            ],
                        }));
                        break outer;
                    ...
                    // Any other status (e.g. "running") is a progress heartbeat — no state change needed
                } catch {
        ...
        // AbortError means a stop action triggered the abort; that action already
        // reset the relevant state, so there is nothing more to do here.
...
export const useVideoStore = create<VideoState & VideoActions>((set, get) => ({
    ...
    uploadVideo: async (file) => {
        ...
        try {
            ...
            set((s) => ({
                ...
                isUploading: false,
                uploadProgress: 100,
                videoErrors: s.videoErrors.filter((e) => e.nodeId !== "reference"),
            }));
        } catch (error: any) {
    ...
    setCompressConfig: (updater) => {
        set((s) => ({
            compressConfig:
                typeof updater === "function" ? updater(s.compressConfig) : updater,
        }));
    },
    ...
    startCompress: async () => {
        ...
        try {
            ...
            if (res.data.status !== "success") {
                const { msg, code, details } = parseApiError(res.data, "Compress failed");
                set((s) => ({
                    isCompressing: false,
                    videoErrors: [
            ...
            await subscribeTaskStream<CompressResult>(taskId, token, {
                ...
                onFailed: () => ({ isCompressing: false, compressTaskId: null }),
                onCancelled: () => ({ isCompressing: false, compressTaskId: null }),
            });
        } catch {
            set((s) => ({
                isCompressing: false,
                videoErrors: [
                    ...s.videoErrors.filter((e) => e.nodeId !== "compress"),
                    makeError("compress", "Network error", "NETWORK_ERROR", NETWORK_ERROR_DETAILS),
                ],
            }));
        }
    },
    ...
    stopCompress: async () => {
        ...
        set({ isCompressing: false, compressTaskId: null, compressResult: null });
    },
    ...
    startAnalyzeScript: async () => {
        ...
        try {
            ...
            if (res.data.status !== "success") {
                const elapsed = (Date.now() - t0) / 1000;
                const { msg, code, details } = parseApiError(res.data, "Extract failed");
                set((s) => ({
                    isExtractingFlow: false,
                    transcriptResult: null,
                    scriptStatus: "error",
            ...
            set((s) => ({
                ...
                videoErrors: [
                    ...s.videoErrors.filter((e) => e.nodeId !== "extracting"),
                    makeError("extracting", "Network error", "NETWORK_ERROR", NETWORK_ERROR_DETAILS),
                ],
            }));
        }
    },
    ...
    startAnalyzeAudio: async () => {
        ...
        try {
            ...
            if (res.data.status !== "success") {
                const elapsed = (Date.now() - t0) / 1000;
                const { msg, code, details } = parseApiError(res.data, "Audio analysis failed");
                set((s) => ({
                    audioStatus: "error",
                    audioTime: elapsed,
                    videoErrors: [
            ...
            await subscribeTaskStream<AudioGlobalFeatures>(taskId, token, {
                controllerKey: "audio",
                t0,
                set,
                nodeId: "audio",
                failureLabel: "Audio analysis failed",
                // Intermediate frames carry streaming audio data alongside status fields.
                onFrame: (frame, s) => {
                    if (frame.asset_id) {
                    ...
                    if (frame.local) {
                        const chunk: AudioStreamChunk = {
                            ...
                            rms: frame.local.rms,
                            spectral_centroid: frame.local.spectral_centroid,
                            spectral_flux: frame.local.spectral_flux,
                            onset_envelope: frame.local.onset_envelope,
                        };
                        set((s) => ({ streamArr: [...s.streamArr, chunk] }));
                    }
                    if (frame.running_global) {
                        set({ audioGlobal: frame.running_global });
                    }
                },
                // The completed result is the final global features; fall back to the
                // last running_global already accumulated in state if result is absent.
                onCompleted: (result, elapsed, s) => ({
                    audioStatus: "success",
                    audioTime: elapsed,
                    audioTaskId: null,
                    audioGlobal: result ?? s.audioGlobal,
                }),
                onFailed: (elapsed) => ({
                    audioStatus: "error",
                    audioTime: elapsed,
            ...
            set((s) => ({
                ...
                videoErrors: [
                    ...s.videoErrors.filter((e) => e.nodeId !== "audio"),
                    makeError("audio", "Network error", "NETWORK_ERROR", NETWORK_ERROR_DETAILS),
                ],
            }));
        }
    },
    ...
    startAnalyzeVisual: async () => {
        ...
        try {
            ...
            if (res.data.status !== "success") {
                const elapsed = (Date.now() - t0) / 1000;
                const { msg, code, details } = parseApiError(res.data, "Visual analysis failed");
                set((s) => ({
                    visualResult: null,
                    visualStatus: "error",
                    visualTime: elapsed,
            ...
            set((s) => ({
                ...
                videoErrors: [
                    ...s.videoErrors.filter((e) => e.nodeId !== "visual"),
                    makeError("visual", "Network error", "NETWORK_ERROR", NETWORK_ERROR_DETAILS),
                ],
            }));
        }
    },
