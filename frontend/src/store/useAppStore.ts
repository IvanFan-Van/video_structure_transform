...
interface AppActions {
    ...
    setConfig: (
        configOrUpdater: ModelConfig | ((c: ModelConfig) => ModelConfig),
    ) => void;
    startTraining: () => void;
    stopTraining: () => void;
...
export const useAppStore = create<AppState & AppActions>((set, get) => ({
        ...
        workerInstance.onmessage = (e: MessageEvent) => {
            ...
            if (type === "dataset_loaded") {
                const suggested = Math.min(
                    3000,
                    Math.max(500, data.numDocs * 3),
                );
                set((state) => ({
                    dataset: data,
                    config: {
                        ...state.config,
                        num_steps: Math.round(suggested / 50) * 50,
                    },
                }));
            }
            if (type === "step") {
    ...
    startTraining: () => {
        ...
        if (workerInstance) {
            ...
            setTimeout(
                () => workerInstance?.postMessage({ type: "train" }),
                50,
            );
        }
    },
    ...
    saveRunHistory: () => {
        set((state) => {
            if (state.modelReady && state.lossHistory.length > 0) {
                const finalLoss =
                    state.lossHistory[state.lossHistory.length - 1].loss;
                const newRun = {
                    id: Date.now(),
                    config: { ...state.config },
                    finalLoss,
